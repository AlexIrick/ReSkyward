...
class ParseData:
    ...
    def only_rows_with_content(self, rows):
        return [{'col': self._headers_row[n+1]['text'], 'row': i} for n, i in enumerate(rows) if i.get('grade')][0] if any(('grade' in i) for i in rows) else {}
    ...
    def handle_tb(self, tb, th_only=False):
        ...
        for d in tb:
            # TABLE HEADER
            if th_only:
                self._headers_row = [self.get_row_info(self.get_h_div(c['h'], 'th')) for c in d['c']]
            ...
            # ASSIGNMENTS
            elif a_name := (a_div := self.get_h_div(d['c'][0]['h'], 'td')).find('a', id='showAssignmentInfo'):
                rows[class_row_index]['assignments'].append({
                    **self.get_assignment_info(a_name.text.strip(), a_div),
                    **self.only_rows_with_content([self.get_assign_row_info(self.get_h_div(c['h'], 'td')) for c in d['c'][1:]])
                })
        return rows
